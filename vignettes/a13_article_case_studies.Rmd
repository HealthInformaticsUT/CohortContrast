...
vignette: >
  %\VignetteIndexEntry{Article Case Study Generation}
...
The workflow uses cohort JSON definitions exported from ATLAS and runs against
the OPTIMA OMOP Common Data Model database.
...
`CohortContrast::cohortFromJSON()` expects a directory containing the JSON
...
When the package is installed, these paths can be resolved with `system.file()`.
...
- The workflow below uses one clean `CohortContrast()` analysis followed by a
  single export of the final filtered and mapped study object to a temporary
  patient-mode folder. That temporary folder is used only to create the
  summary-mode study and is then removed. The retained study folders therefore
  contain only the final concepts that passed the statistical selection steps.
...
`precomputeSummary()` is running and are removed afterward.

The retained summary-mode study folders therefore contain the final published
outputs.
