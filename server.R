  ...
  output$prevalencePlot <- shiny::renderPlot({
    ...
    result <- tryCatch({
      if (is.null(studyName()) | is.null(target_filtered())) {
        getErrorPlot("Please select a study from the dashboard")
      } else {
        if (isPatientLevelDataPresent()) {
...
createPrevalencePlot(filtered_target = NULL, isCorrelationView = FALSE)
  ...
  output$heatmapPlot <- shiny::renderPlot({
    ...
    result <- tryCatch({
      ...
      print(e)
      createHeatmapPlot(filtered_target = NULL, isCorrelationView = FALSE)
    })
  ...
  output$trajectoryGraph <- visNetwork::renderVisNetwork({
    ...
    if (is.null(target_filtered()) ||
      ...
      getErrorPlot("Required inputs are missing for the trajectory graph")
    } else {
      trajectoryPlotWaiter$show()
      result <- createCorrelationTrajectoryGraph(
        target_filtered(),
  ...
  output$trajectoryGraphPlot <- shiny::renderPlot({
    if (is.null(studyName()) || is.null(target_filtered())) {
      ...
      createCorrelationTrajectoryGraph(
        target_filtered(),
        selectedCorrelationGroup(),
        input$edge_prevalence_threshold,
        selected_trajectory_filtering_values(),
  ...
  shiny::observeEvent(input$accept_btn_visual_snaphot, {
    ...
    shiny::removeModal()
    createPathToResults(paste0(pathToResults, "/visual_snapshots"))
    fileName <- paste(pathToResults, "/visual_snapshots/",sanitize_single(input$new_visual_snapshot_name),".rds", sep = "")
    config <- cachedData()$config
    ...
    temp_filtered_target = prepare_filtered_target(temp_filtered_target, correlationCutoff())
    ccObject <- list(
    ...
    save_object(object = ccObject, path = fileName)
  ...
  shiny::observeEvent(input$save_btn, {
    ...
    if(isPatientLevelDataPresent()){
      ...
      createPathToResults(paste0(pathToResults, "/snapshots"))
      ...
      save_object_metadata(object = ccObject, path = file_path, studyName = file_study_name)
      # Notify the user
