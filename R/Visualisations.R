...
format_results <-
  function(data,
    ...
    data_features_temp <-
      data$data_features[get("ABSTRACTION_LEVEL") == abstractionLevel, list(
        get("CONCEPT_ID"),
        get("ZTEST"),
        get("LOGITTEST"),
        get("TIME_FIRST"),
    ...
    data$data_features[, get("PREVALENCE_DIFFERENCE_RATIO") := data.table::fifelse(
      ...
      data.table::fifelse(
        (is.na(get(
          ...
          is.na(get("TARGET_SUBJECT_PREVALENCE")),-1,
        data.table::fifelse(
    ...
    if (autoScaleRate) {
      ...
      scaled_prevalence <-
        merge(data$data_patients[get("ABSTRACTION_LEVEL") == abstractionLevel],
    ...
    wider_dt <-
      wider_dt[data$data_features[, list(get("CONCEPT_ID"),
                                         get("TIME_FIRST"),
                                         get("TIME_MEDIAN"),
                                         get("TIME_LAST"))], nomatch = 0]
    ...
    target_df <-
      data.table::as.data.table(wider_dt["CONCEPT_ID" != 999999999 &
                                           "CONCEPT_ID" != "0"])
    ...
    target_time_annotation <- data$data_patients[get("COHORT_DEFINITION_ID") == "target" &
...
plot_time <- function(filtered_target) {
  ...
  concept_ordering <- filtered_target$target_row_annotation %>% dplyr::arrange(dplyr::desc(.data$PREVALENCE_DIFFERENCE_RATIO)) %>% dplyr::pull(.data$CONCEPT_NAME)

  plot_data <- plot_data %>%
    dplyr::mutate(CONCEPT_NAME = factor(.data$CONCEPT_NAME, levels = concept_ordering)) %>%
