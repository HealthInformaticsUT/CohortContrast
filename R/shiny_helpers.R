...
format_results <-
  function(data,
    ...
    if (applyChi2YTest & applyLogitTest) {
      data_copy$data_features <- data_copy$data_features[
        data_copy$data_features$CHI2Y == TRUE | data_copy$data_features$LOGITTEST == TRUE, ]
    } else if (applyChi2YTest) {
      data_copy$data_features <- data_copy$data_features[
        data_copy$data_features$CHI2Y == TRUE, ]
    ...
    if (autoScaleRate) {
      ...
      data_copy$data_features <-
        updateFeatures(data_copy$data_features, scaled_prevalence)[, .(
          CONCEPT_ID,
          CONCEPT_NAME,
    ...
    wider_dt <- data.table::dcast(
      target_dt,
      CONCEPT_ID + CONCEPT_NAME + HERITAGE + PREVALENCE_DIFFERENCE_RATIO + PREVALENCE + CHI2Y + LOGITTEST  ~ PERSON_ID,
      value.var = "PRESENT",
      fill = 0
    )
...
#' @keywords internal
updateFeatures <- function(features, scaled_prev) {
