...
nGramDiscovery <- function(data, collapse_size = 0) {
  ...
  all_phases <- runningNgrams(collapsed_long, map_v)
  ...
  clustered <- runClustering(all_phases)
...
#' @keywords internal
preprocessData <- function(data, collapse_size = 0) {
  features <- data$data_features %>%
    dplyr::filter(ABSTRACTION_LEVEL == -1, CHI2Y, TARGET_SUBJECT_PREVALENCE > 0.01)
...
#' @keywords internal
findEnrichedNgrams <- function(data, n = 2, prev_probs = 1) {
  stopifnot(n >= 2)
  ...
  observed <- filtered %>%
    ...
    dplyr::summarise(
      grams = list({
        ids <- CONCEPT_ID
        times <- TIME_TO_EVENT
        ngrams <- extractNgrams(ids, n)
        timegrams <- extractNgrams(times, n)
        if (is.null(ngrams)) {NULL}
        tibble::tibble(
          ngram = makeNgram(ngrams),
...
#' @keywords internal
runningNgrams <- function(collapsed_long, map_v, global_prev = 0.01) {
  ...
  while (TRUE) {
    ...
    result <- findEnrichedNgrams(collapsed_long, n = n_val, prev_probs = global_prev)
