...
buildTargetTable <- function(cdm, jsonDir) {
  ...
  targetTable <- CohortContrast::cohortFromJSON(
    ...
    dplyr::mutate(
      cohort_start_date = cohort_start_date - 14,
...
buildVisitBasedControl <- function(cdm, targetTable, lookbackDays = 1095, followupDays = 365) {
  # Shift the target windows backwards to define the control-search anchor.
  controlSeed <- targetTable %>%
    dplyr::mutate(
      cohort_start_date = cohort_start_date - lookbackDays,
      cohort_end_date = cohort_start_date + followupDays
    )
  ...
  controlTable <- cdm$visit_occurrence %>%
    ...
    dplyr::transmute(
      ...
      cohort_end_date = visit_start_date + followupDays
    ) %>%
    dplyr::ungroup() %>%
    dplyr::collect()
